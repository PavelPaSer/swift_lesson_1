...
var Arr = Array(repeating: String(someChar), count: someStr.count)
print(Arr) //["S", "S", "S", "S", "S"]
type(of: Arr)
someStr = String(someChar)
...
let newStr = "JonhWick"
let secondIndex = newStr.index(newStr.startIndex, offsetBy: 0)
print(newStr[secondIndex])
let lastIndex = newStr.index(before: newStr.endIndex)
newStr[lastIndex]
newStr.unicodeScalars.count

let newsStrg = "БабаЯга"
let fristIndex = newsStrg.startIndex
let charIndex = newsStrg.index(fristIndex, offsetBy: 3)
let subStrt = newsStrg[fristIndex...charIndex]
type(of: subStrt)
...
let textWhite: Set<Character> = ["\u{2657}", "\u{2656}", "\u{2657}"]
let textBlack: Set<Character> = ["\u{265A}", "\u{265B}", "\u{265C}"]
 */
